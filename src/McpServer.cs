    ...
    private Task<object> HandleListToolsAsync(int? id)
    {
        var tools = new List<object>
        {
            ...
            (object)new
            {
                ...
                description = "Search for types, methods, properties, etc. by name across the solution. Supports glob patterns (e.g., '*Handler' finds classes ending with 'Handler', 'Get*' finds symbols starting with 'Get'). Use ? for single character wildcard. Returns symbol name, kind, namespace, and location.",
                inputSchema = new
                {
                    type = "object",
                    properties = new
